...
(async function () {
  ...
  const RFT_LOAD_INSTANCES = tape && tape.loadTapeInstances;
  const RFT_SAVE_INSTANCES = tape && tape.saveTapeInstances;
  if (!tape || !RFT_EVALUATE || !RFT_LOAD_INSTANCES || !RFT_SAVE_INSTANCES) { console.warn('[RecallFox/Tape] lib tape tidak tersedia — skip'); return; }
