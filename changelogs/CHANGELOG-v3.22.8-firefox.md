# CHANGELOG — RecallNote/RecallTape v3.22.8 (Firefox)
...
## Fix (content/notes-cs.js, content/tape-cs.js) — paritas penuh dengan Chrome
...
- Firefox: `notes-cs.js`, `tape-cs.js`, `sidebar-cs.js` (floater pill) terdaftar untuk
  `file:///*` + `http/https` (termasuk localhost) dan berjalan out-of-the-box —
  konten note kini tersinkron global ke semua halaman tersebut.
- **Viewer PDF bawaan (pdf.js)** tidak bisa menerima DOM injection (viewer
  berprivilege) — floating note tidak dapat tampil di tab PDF, di browser mana pun.
  Gunakan halaman HTML/localhost, atau buka sidebar RecallFox di tab PDF.

## Validasi

- `node --check` 63 file JS Firefox + 57 file Chrome = 0 error.
...
- Regresi: `ff_sim` 10/10 PASS, `chrome_sw_sim` 14/14 PASS, `echo_sim2` (fix v3.22.7
