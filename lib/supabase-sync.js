...
function _parseNoteRow(row) {
  return {
    ...
    pinned: !!row.pinned,
    archived: !!row.archived,
    // v3.19.1: source JSONB untuk voice notes metadata (audioUrl, kind, duration, location).
    // Kompatibel dengan PWA v1.8.0 yang simpan source.kind='voice' + source.audioUrl.
    source: row.source || null,
    createdAt: row.created_at || new Date().toISOString(),
    updatedAt: row.updated_at || new Date().toISOString()
  };
