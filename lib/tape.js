...
//   + 19% PPN                      → add 19% of running total (percent)
...
// ============================================================================
// Line parsing — split into { op, amount, note, raw, isPercent }
// ============================================================================
 ...
 *   parseLine('+ 19% PPN')             → { op:'+', amount:19, isPercent:true, note:'PPN', raw:'...' }
 *   parseLine('')                      → null
 */
export function parseLine(rawLine) {
  ...
  if (op === '=') {
    ...
    if (m) {
      ...
      if (amt != null) {
        return { op, amount: amt, isPercent: !!m[2], note: (m[3] || '').trim(), raw };
      }
    }
  ...
  if (!m) {
    ...
    return { op: 'note', amount: null, isPercent: false, note: rest, raw };
  }
  const amtStr = m[1].trim();
  const isPercent = !!m[2];
  const note = (m[3] || '').trim();
 ...
 *   {
 *     entries: [
 *       { op, amount, note, raw, running: number, display: number, kind: 'op'|'subtotal'|'note'|'comment', isPercent, percentValue }
 *     ],
 *     grandTotal: number,
 *     error: string | null
 *   }
 ...
 *   - percent (+/-):  running += running * (amt/100) or running -= running * (amt/100)
 */
...
export function toPlainText(tape, opts = {}) {
  ...
  for (const e of entries) {
    ...
    const note = e.note || '';
    lines.push(`${sym} ${amtStr.padStart(amountCol)}${hint}  ${note}`.trimEnd());
  }
  lines.push('  ' + '═'.repeat(amountCol + 4));
...
export function selfTest() {
  ...
  // Percent tests — verify parseLine + percentValue computation
  const percentTests = [
  ...
  // Edge case: '4 Bagi 4 orang' should NOT parse 'b' as billion suffix
  const edgeTests = [
    { input: '4 Bagi 4 orang', expectedOp: '+', expectedAmount: 4, expectedNote: 'Bagi 4 orang' },
    { input: '+ 50k Bonus', expectedOp: '+', expectedAmount: 50000, expectedNote: 'Bonus' },
    { input: '= Subtotal', expectedOp: '=', expectedNote: 'Subtotal' },
    { input: '# comment', expectedOp: 'comment' },
    { input: 'note only', expectedOp: 'note' },
    { input: '', expectedNull: true },
  ];
  const edgeResults = edgeTests.map(t => {
    const got = parseLine(t.input);
    let ok;
    if (t.expectedNull) ok = got === null;
    else {
      ok = got && got.op === t.expectedOp;
      if (t.expectedAmount != null) ok = ok && got.amount === t.expectedAmount;
      if (t.expectedNote != null) ok = ok && got.note === t.expectedNote;
    }
    return { input: t.input, got, ok };
  });

  const allOk = results.every(r => r.ok) && percentResults.every(r => r.ok) && edgeResults.every(r => r.ok);
  return { allOk, results, percentResults, edgeResults };
}
