...
export async function deleteItem(id) {
  ...
  // v3.11.29: Supabase delete sync — kirim soft-delete ke cloud
  // supaya device lain tahu item ini dihapus (realtime via deleted_at column).
  ...
  // v3.11.31: Tambah ke delete registry (via background, supaya tidak import cycle)
  // Pakai dynamic import supaya storage.js tidak depend langsung ke supabase-sync.js
  ...
  // v3.11.29: Trigger auto-sync push juga (untuk push perubahan lokal lainnya)
  browser.runtime.sendMessage({ type: 'SUPABASE_AUTO_SYNC' }).catch(() => {});
}
...
export async function deleteBundle(id) {
  ...
  // v3.11.29: Supabase delete sync — bundle is stored as vault_item type='bundle'
  browser.runtime.sendMessage({
    type: 'SUPABASE_DELETE_ITEM',
    itemId: id,
    itemType: 'bundle'
...
export async function deleteNote(id) {
  ...
  // v3.11.31: Tambah ke delete registry lokal
  import('./supabase-sync.js').then(mod => {
    if (mod.addNoteToDeleteRegistry) {
      mod.addNoteToDeleteRegistry(id, new Date().toISOString()).catch(() => {});
    }
  }).catch(() => {});
  browser.runtime.sendMessage({ type: 'SUPABASE_AUTO_SYNC' }).catch(() => {});
}
