...
const DEFAULT_SETTINGS = {
  ...
  // v3.11.2 (Issue 2): Profanity filter mode — ganti "mode anak (filter konten)" logikanya.
  // Sebelumnya: redirect/filter YouTube Kids. Sekarang: nuclear mode yang blokir kata kasar
  // Indonesia (anjir, cok, bangsat, kontol, dll.) di semua halaman web.
  // Dipicu oleh keluhan user: "mode anak (filter konten) itu logikanya diganti menjadi seperti
  // 'nuclear mode' tapi memblokir kata kata tidak sopan seperti anjir, cok, dsb."
  contentGuardProfanityMode: false,         // master switch — blokir kata kasar di semua halaman
  contentGuardProfanityMask: true,          // mask kata kasar dengan *** (true) vs hide elemen (false)
  contentGuardProfanityQuizArmed: 0,        // epoch-ms hingga profanity mode aktif (quiz gate)
  // v3.11.2 (Issue 2): Quiz gate — kalau enabled, untuk MEMATIKAN mode ini user harus jawab
  // kuis matematika (supaya anak-anak ga bisa sembarang off).
  contentGuardQuizGate: true,               // default ON — quiz required to disable kid/profanity mode

  // v3.11.2 (Issue 1): Pomodoro timer — pengganti "kotak merah" yang user minta diganti.
  // Settings dasar disimpan di lib/pomodoro.js (DEFAULT_POMODORO), tapi flag ini kontrol
  // apakah pomodoro widget tampil di strip bar (default false — user harus explicit enable).
  pomodoroEnabled: false,
  pomodoroShowInStrip: false,               // tampilkan mini counter di strip bar (sebagai pengganti prayer)

  // v3.11.2 (Issue 3): Hide tiles — user bilang "kotak ijo jarang dipake, sembunyikan aja".
  // Default false (tiles tetap tampil), user bisa hide via Settings.
  hideHeroTiles: false,

  // v3.11.2 (Issue 3): Drag-drop reorder untuk tools + tabs.
  // Array of tool IDs in custom order. Empty array = pakai default order.
  toolOrder: [],
  tabOrder: [],                              // ['home', 'notes', 'tools']
