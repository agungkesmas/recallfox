...
export function buildTree(items, expandedIds, categoryFilter, showGroups) {
  if (!showGroups) {
    // Flat mode — "Semua" tab. Group items TIDAK tampil sama sekali.
    const flat = items
      .filter(it => !isGroupItem(it))
      .map(it => ({ kind: 'item', item: it }));
    return flat;
  }

  // Tree mode — kategori spesifik (Prompt, Link, Media, dll)
  const groups = [];
  const topLevel = [];
  ...
  for (const g of groups) {
    ...
    // Skip empty groups (no children matching filter)
