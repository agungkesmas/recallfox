...
export function buildTree(items, expandedIds, categoryFilter, showGroups) {
  // v3.18.4: Build index — semua item by ID, semua children by parent ID
  const allByParent = new Map();  // parentId → [item, ...] (includes groups AND items)
  const topLevel = [];  // items + groups dengan parentId = null
  ...
  // Filter top-level by category (hanya kalau categoryFilter aktif)
  ...
  // Sort: groups first, then by createdAt
  ...
  function buildNode(it) {
    if (isGroupItem(it)) {
      ...
      // Get all children (items + sub-groups)
      let children = (allByParent.get(it.id) || [])
        .sort((a, b) => {
          const ag = isGroupItem(a) ? 0 : 1;
          const bg = isGroupItem(b) ? 0 : 1;
          if (ag !== bg) return ag - bg;
          return getOrder(a) - getOrder(b) || new Date(a.createdAt || 0) - new Date(b.createdAt || 0);
        });
