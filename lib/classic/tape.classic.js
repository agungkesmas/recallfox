// v3.22.5-firefox FIX-3b (regen v3.23.0): IIFE idempoten — semua content script
// satu extension berbagi satu isolated world. Tanpa IIFE, const/function
// top-level dari bundle lain dengan nama sama menyebabkan SyntaxError
// redeclarasi atau silent override lintas bundle.
(function () {
  if (globalThis.__RF_LIB_TAPE__) return; // idempoten: sudah pernah dimuat
// lib/tape.js — RecallTape helpers (parse/evaluate/format, zero dependencies)
 ...
 */
function toPlainText(tape, opts = {}) {
...
async function saveSession(text) {
  try {
    await browser.storage.local.set({ [SESSION_KEY]: text });
  } catch (e) {}
}

async function savePinState(pinned) {
  try {
...
async function loadTapeInstances() {
  try {
...
async function saveTapeInstances(list) {
  try {
...
globalThis.__RF_LIB_TAPE__ = { evaluate, formatCurrency, formatNumber, loadSession, loadTapeInstances, parseAmount, parseLine, savePinState, saveSession, saveTapeInstances, selfTest, toMarkdown, toPlainText };
try { window.__RF_LIB_TAPE__ = globalThis.__RF_LIB_TAPE__; } catch (e) {}
