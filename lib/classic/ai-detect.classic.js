...
(function () {
  if (globalThis.__RF_LIB_AIDETECT__) return; // idempoten: sudah pernah dimuat
 ...
 */
async function migrateFromAiTools(settings) {
  try {
 ...
 */
async function isAIPageFromOrigin(origin) {
  try {
 ...
 */
async function getMatchedSite(origin) {
  try {
    const aiSites = await getAiSites();
    for (const site of aiSites) {
      if (!site.active) continue;
 ...
 */
async function removeAiSite(id) {
  try {
    const aiSites = await getAiSites();
 ...
 */
async function toggleAiSite(id, active) {
  try {
    const aiSites = await getAiSites();
 ...
 */
async function updateAiSite(id, patch) {
  try {
    const aiSites = await getAiSites();
    const site = aiSites.find(s => s.id === id);
    if (site) {
...
globalThis.__RF_LIB_AIDETECT__ = { addAiSite, getAiSites, getMatchedSite, isAIPage, isAIPageFromOrigin, logAI, matchDomain, migrateFromAiTools, removeAiSite, toggleAiSite, updateAiSite };
try { window.__RF_LIB_AIDETECT__ = globalThis.__RF_LIB_AIDETECT__; } catch (e) {}
