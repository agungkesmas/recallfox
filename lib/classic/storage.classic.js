...
async function setScreenshotBlob(id, dataUrl) {
...
async function setFileDataUrl(id, dataUrl) {
...
async function importScreenshotBlobs(map) {
...
async function addBundle(name, itemIds = [], opts = {}) {
  const vault = await getVault();
...
async function updateBundle(id, patch) {
  const vault = await getVault();
...
async function reassignToBundle(bundleId, itemId, action = 'add', itemType = 'item') {
  const vault = await getVault();
...
async function mergeSyncIntoLocal() {
...
async function addCustomTopping(topping) {
  const vault = await getVault();
  if (!vault.toppings) vault.toppings = [];
  const newTopping = {
...
async function updateCustomTopping(id, patch) {
  const vault = await getVault();
  if (!vault.toppings) vault.toppings = [];
...
async function deleteCustomTopping(id) {
  const vault = await getVault();
  if (!vault.toppings) vault.toppings = [];
...
async function addNote(body = '', opts = {}) {
...
async function addUserBlocklistEntry(entry) {
  const vault = await getVault();
...
async function removeUserBlocklistEntry(id) {
  const vault = await getVault();
...
async function clearUserBlocklist() {
  const vault = await getVault();
  vault.settings.contentGuardUserBlocklist = [];
  await saveVault(vault);
  return { ok: true };
