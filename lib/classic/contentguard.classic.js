...
const DEFAULT_BLOCKED_YT_CHANNELS = [
...
const DEFAULT_BLOCKED_X_ACCOUNTS = [
...
const DEFAULT_CHINA_YOUTUBE_SEARCHES = [
...
const DEFAULT_CHINA_X_ACCOUNTS = [
...
const DEFAULT_CHINA_X_SEARCHES = [
...
function containsNegativeKeyword(text, keywords = DEFAULT_NEGATIVE_KEYWORDS) {
...
function detectSearchQuery(url) {
  try {
    const u = new URL(url);
...
function matchesBlockedSearchQuery(query, blockedQueries = DEFAULT_BLOCKED_SEARCH_QUERIES) {
...
function matchesIdNewsDomain(url, domains = DEFAULT_ID_NEWS_DOMAINS) {
  try {
    const u = new URL(url);
    const host = u.hostname.toLowerCase();
...
function matchesUserBlocklist(text, channel, blocklist) {
...
function matchesBlockedXPostUrl(postUrl, blocklist) {
...
const DEFAULT_BLOCKED_SEARCH_QUERIES = [
  // Politisi
...
const CONTENTGUARD_DEFAULT_SETTINGS = {
 ...
 */
function buildProfileMatchCache(profile) {
 ...
 */
function matchesActiveProfile(title, channel, cache) {
 ...
 */
function matchesProfileSearchQuery(query, profile) {
 ...
 */
function getActiveProfile(topicProfiles) {
 ...
 * @returns {boolean}
 */
function isProfileFiltering(profile) {
 ...
 */
function seedDefaultTopicProfiles(existing) {
