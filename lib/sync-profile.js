...
export async function pushStateToCloud(profile) {
  ...
  const syncUrl = profile.url + (profile.url.includes('?') ? '&' : '?') + 'action=sync_state&alt=json';
...
export async function pullStateFromCloud(profile) {
  ...
  const pullUrl = profile.url + (profile.url.includes('?') ? '&' : '?') + 'action=get_state&profile=' + encodeURIComponent(profile.name) + '&alt=json';
  const controller = new AbortController();
