...
export async function uploadScreenshot(screenshotItem, dataUrl) {
  ...
  const metadata = {
    ...
    screenshotFormat: screenshotItem.screenshotFormat,
    screenshotBytes: screenshotItem.screenshotBytes,
    thumbnailDataUrl: (screenshotItem.thumbnailDataUrl || '').slice(0, 2048), // batasi thumbnail
  ...
  formData.append('file', blob, 'screenshot.' + (mimeType === 'image/jpeg' ? 'jpg' : 'png'));

  try {
    const res = await fetch(webAppUrl, {
      method: 'POST',
      body: formData
      // Jangan set Content-Type — browser akan set dengan boundary otomatis untuk FormData
    });
